...
isimage = lambda s: (s.dtype == np.uint8 or s.dtype==np.uint16) and len(s.shape) == 3
# isimage = lambda s: (np.issubdtype(s.dtype, np.uint16) and  len(s.shape) == 3)
...
# Depth: float32 → 0‒1 → 0‒255 회색톤 → 3-채널
def depth_to_u8(x):
    x = jnp.clip(x, 0, 1)                 # Dreamer 출력 범위 가정
